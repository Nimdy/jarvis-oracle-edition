...
_KIND_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bwhat can you do\b|\bwhat are you (capable|able)\b|\byour (capabilit|abilit|architecture|codebase|subsystem|design)\b|\b(describe|tell me about|explain|walk me through|what can you tell me about)\b.{0,25}\b(your|the)?\b.{0,6}\b(architecture|codebase|subsystem|design|how you (work|reason|think|get|produce|generate|answer))\b|\bhow (do|are) you (work|built|structured|made|put together)\b|\bhow do you (get|reach|produce|generate|come up with|arrive at) an? answer\b|\b(explain|describe|tell me about|walk me through|how (?:does|do)|how(?:'?s| is))\b.{0,20}\b(your|the|this)\b.{0,8}\b(system|inner workings|internals|machinery|wiring|build)\b|\bhow (your|the) system (work|is built|runs)", re.I), "capabilities"),
