...
**The keystone disease:** all of it points **inward**. The "win" signal is the system's own 8 internal health metrics and the self-graded Oracle benchmark — producing **~6507 metric-created goals** whose top recurrent motive is the tautological *"fix my own shadow_default_win_rate,"* with `orphan_rate 0.857`, `avg_chain_length 1.0`, and a `grounded:inferred` memory ratio of `3.3×`. A mirror: **curiosity that rewards internal consistency cannot discover it is wrong about the world.**
...
- **`grounded:inferred`** ratio trending **down** from 3.3×.
