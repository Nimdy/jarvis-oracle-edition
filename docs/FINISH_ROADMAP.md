...
**✅ Environmental memory-of-normal (shadow) SHIPPED (2026-06-01, commit 732e62d) — the "be there for the room" half (north-star: the dignity-anchor's "your keys aren't where they live").** `consciousness/environmental_normal.py`: learns each object's USUAL spot (dominant semantic region) + usual presence by quiet observation (fed each 60s scene-continuity tick), then would gently note a real, confident deviation ("the cup isn't in its usual spot — it usually sits on the desk, now on the left") — NEVER spoken, hypothesis, salience-gated. **Must KNOW not guess:** a usual spot is only trusted after ≥15 obs in a ≥70%-dominant region; below that JARVIS stays silent. A LIGHTWEIGHT per-label accumulator on the existing scene entities — does NOT require the PRE-MATURE hrr_scene mind's-eye, does NOT touch the novel-object ask. Persisted (~/.jarvis/environmental_normal.json) so the normal accrues across days. Surfaced on v2/grounding. Verified locally: learns the spot, flags a real move + absence, quiet in place, refuses to flag an unearned object. **Next "be there" increments (earn-gated):** companion P3 (read→gentle *spoken* presence) once the shadow reads prove trustworthy; deeper environmental (bbox-level position within a region, object-pair relations) as the spatial mind's-eye matures.
