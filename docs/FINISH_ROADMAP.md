...
| # | Phase | Type | Action |
|---|-------|------|--------|
| 21 | P0 | build_shadow | Finish origin telemetry — `origin_is_synthetic` on TeacherSignal; per-teacher lived/synthetic counts; `accuracy_history` deque[float]→deque[dict]{ts,outcome,origin} + `get_accuracy(origin=live)`. (Leak already guarded.) |
| 22 | P1 | build_shadow | Compute `live_shadow_accuracy` per specialist (origin=live only, min-N gated); fill the `0.0` stub at `snapshot.py:2632`; surface synthetic vs live on the lab panel. |
