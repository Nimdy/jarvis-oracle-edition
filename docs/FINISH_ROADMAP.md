...
**✅ Environmental memory-of-normal (shadow) SHIPPED — FINAL = Path-1, data-minimal "moved-only" (2026-06-01, commit 03cbe26, after 3 adversarial workflow reviews; supersedes v1 732e62d / v2 8040127) — the "be there for the room" half (north-star: the dignity-anchor's "your keys aren't where they live").** `consciousness/environmental_normal.py`. Makes EXACTLY ONE claim: **"moved"** — a KNOWN object CONFIDENTLY (≥0.50) FRESHLY SEEN in a region ≠ its learned usual spot, debounced ≥2 ticks (keyed on label+kind+region, anti-jitter), suppressed if also seen in its usual spot. It **NEVER claims absence** ("gone") — that would need person-occlusion data we deliberately DON'T wire (sovereignty / data-minimization, David's call; "moved to the wrong spot" is the honest higher-value nudge). Learns the usual spot from what the tracker still BELIEVES EXISTS (permanence ≥0.40), ≥15 obs + ≥70% dominance; exponential forgetting (re-learns a real relocation); snapshot-staleness guard; persisted (version=3). **It FAILS SAFE — cannot confabulate absence; "moved" needs a confident real sighting.**

**✅ PERSON-OCCLUSION AWARENESS WIRED + VERIFIED LIVE (2026-06-01, commits 37acf58 + 194df8e) — David chose "the real fix".** Was the root blocker across all 3 reviews: `person_bboxes=[]` was hardcoded (Pi stripped persons) → `occluded` state structurally unreachable → objects phantom-removed the instant a hand crossed the desk. Fixed with 5 tiny backward-compatible seams (pi/main.py → pi/transport/event_schema.py → brain/perception/server.py → perception_orchestrator._on_scene_summary → scene_tracker.update). **Sovereignty (verified live):** person bboxes are transient occlusion GEOMETRY only — never an entity, never persisted, never identity. **Verified live:** `occluded` now appears, `region_visibility` drops ~0.03–0.4 under a person, **person-leak into entities = 0**, env-normal `learned` went 0→3. Tuned `_PERM_FLOOR`→0.34; maturity now `occlusion_live_coarse_sampling`. This was a genuine ENVIRONMENTAL-AWARENESS fix ("aware of surroundings"), benefiting the whole world-model. **Remaining honest limits (in get_status `limitations[]`):** ~60s accrual sampling + coarse 3-region grid w/o hysteresis → learning takes minutes-to-hours. **Lesson:** local unit tests passed on hand-fed `occluded` entities that never occur live — adversarial cloud review + live verification caught it in SHADOW before a word was spoken. Pi has NO restart endpoint: deploy = sync-pi.sh + operator `./start.sh`.

**Next "be there" increments (earn-gated):** companion P3 (read→gentle *spoken* presence) once the shadow reads prove trustworthy; the perception-substrate maturation above unlocks the room-half's real usefulness (+ the absence half, if ever wanted).
